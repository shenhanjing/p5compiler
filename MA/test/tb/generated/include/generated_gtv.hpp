...
public:
    ...
    FvPackedBuffer pack_fv_to_bytes() const {
        std::vector<bool> bits;
...
protected:
    ...
    static void append_bits(std::vector<bool> &bits, const P5UInt &value, std::size_t width) {
    ...
    static void assign_from_bits(const Buffer &buf, std::size_t &cursor, std::size_t width, P5UInt &target) {
        uint64_t v = 0;
