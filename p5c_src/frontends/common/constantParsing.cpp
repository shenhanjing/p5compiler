...
static IR::Constant *parseConstantWithWidth(Util::SourceInfo srcInfo, const char *text,
    ...
    // Standalone p5compiler does not carry full backend configuration.
    // Keep a conservative hard limit to avoid pathological allocations.
    constexpr long kMaxWidthSupported = 65535;
    if (size > kMaxWidthSupported) {
