...
void ProgramStructure::checkHeaderType(const IR::Type_StructLike *hdr, bool metadata) {
    ...
    for (auto f : hdr->fields) {
        if (f->type->is<IR::Type_Varbits>()) {
            if (metadata)
                ::P4::error(ErrorType::ERR_INVALID, "%1%: varbit types illegal in metadata", f);
...
bool ProgramStructure::isFieldInList(cstring type, cstring field, const IR::FieldList *fl) const {
    ...
    for (auto e : fl->fields) {
        if (auto mem = e->to<IR::Member>()) {
            ...
            return isFieldInList(type, field, l);
...
const IR::Expression *ProgramStructure::listIndex(const IR::Expression *expression) const {
    ...
    if (pe == nullptr) {
        ::P4::error(ErrorType::ERR_EXPECTED, "%1%: Expected a field list", expression);
        return 0;
    }
        ...
        if (primitive->name == "extract") {
            ...
            if (!destType->is<IR::Type_Header>()) {
                ::P4::error(ErrorType::ERR_INVALID,
                            "%1%: invalid argument. Expected a header not %2%", primitive,
                            destType->toString());
                return nullptr;
            }
            auto finalDestType =
                ::P4::get(finalHeaderType, destType->to<IR::Type_Header>()->externalName());
...
const IR::PathExpression *ProgramStructure::getState(IR::ID dest) {
    ...
    if (ps != nullptr) {
        ...
        if (ctrl != nullptr) {
            ...
            ::P4::error(ErrorType::ERR_NOT_FOUND, "%1%: unknown state", dest);
            return nullptr;
        }
    }
}
...
void ProgramStructure::createParser() {
    ...
    if (states.empty()) ::P4::error(ErrorType::ERR_INSUFFICIENT, "No parsers specified");
...
void ProgramStructure::createParserE() {
    // Create parser_e identical to parser_i for P4_14 compatibility
    // P4_14 only has one parser, so we duplicate it for both parser_i and parser_e
    auto paramList = new IR::ParameterList;
    
    auto pinpath = new IR::Path(p4lib.packetIn.Id());
    auto pintype = new IR::Type_Name(pinpath);
    auto packetParam = new IR::Parameter(v1model.parser.packetParam.Id(), IR::Direction::None, pintype);
    paramList->push_back(packetParam);

    auto headpath = new IR::Path(v1model.headersType.Id());
    auto headtype = new IR::Type_Name(headpath);
    auto headersParam = new IR::Parameter(v1model.parser.headersParam.Id(), IR::Direction::Out, headtype);
    paramList->push_back(headersParam);

    auto metapath = new IR::Path(v1model.metadataType.Id());
    auto metatype = new IR::Type_Name(metapath);
    auto metaParam = new IR::Parameter(v1model.parser.metadataParam.Id(), IR::Direction::InOut, metatype);
    paramList->push_back(metaParam);

    auto stdMetaPath = new IR::Path(v1model.standardMetadataType.Id());
    auto stdMetaType = new IR::Type_Name(stdMetaPath);
    auto stdMetaParam = new IR::Parameter(v1model.ingress.standardMetadataParam.Id(), 
                                          IR::Direction::InOut, stdMetaType);
    paramList->push_back(stdMetaParam);

    auto type = new IR::Type_Parser(IR::ID("Parser_E_Impl"), new IR::TypeParameters(), paramList);
    
    // Use the same parser states as ParserImpl
    IR::IndexedVector<IR::Declaration> stateful;
    IR::IndexedVector<IR::ParserState> states;
    for (auto p : parserStates) {
        auto ps = convertParser(p.first, &stateful);
    ...
    if (!states.empty()) {
        auto result = new IR::P4Parser(IR::ID("Parser_E_Impl"), type, stateful, states);
        declarations->push_back(result);
    }
}
        ...
        if (expression->is<IR::ConcreteHeaderRef>()) {
            ...
            if (auto pe = hsir->index_->to<IR::PathExpression>()) {
                if (pe->path->name.name == "next") {
                    ...
                    ::P4::error(ErrorType::ERR_EXPRESSION, "%1%: Illegal extract stack expression",
                                pe);
                    return hdr;
                }
            }
...
void ProgramStructure::createDeparserInternal(
    ...
    if (loop)
        ::P4::warning(ErrorType::WARN_ORDERING,
...
const IR::Declaration_Instance *ProgramStructure::convertActionProfile(
    ...
    if (!action_profile->selector.name.isNullOrEmpty() && !action_selector)
        ::P4::error(ErrorType::ERR_NOT_FOUND, "Cannot locate action selector %1%",
...
const IR::P4Table *ProgramStructure::convertTable(const IR::V1Table *table, cstring newName,
    ...
    if (table->reads != nullptr) {
        ...
        if (action_selector != nullptr) {
            auto flc = field_list_calculations.get(action_selector->key.name);
            if (flc == nullptr) {
                ::P4::error(ErrorType::ERR_NOT_FOUND, "Cannot locate field list %1%",
                            action_selector->key);
            } else {
                auto fl = getFieldLists(flc);
                if (fl != nullptr) {
...
const IR::Expression *ProgramStructure::convertHashAlgorithm(Util::SourceInfo srcInfo,
    ...
    if (algorithm == "crc32") {
        ...
        ::P4::warning(ErrorType::WARN_UNSUPPORTED, "%1%: unexpected algorithm", algorithm);
        result = algorithm;
    }
...
static bool sameBitsType(const IR::Node *errorPosition, const IR::Type *left,
    ...
    ::P4::error(ErrorType::ERR_TYPE_ERROR, "%1%: operation only defined for bit/int types",
                errorPosition);
...
const IR::Statement *ProgramStructure::sliceAssign(const IR::Primitive *primitive,
    ...
    if (mask->is<IR::Constant>()) {
        ...
        if (cst->value < 0) {
            ::P4::error(ErrorType::ERR_INVALID, "%1%: Negative mask not supported", mask);
            return nullptr;
        }
        if (cst->value != 0) {
...
static const IR::Constant *push_pop_size(ExpressionConverter &conv, const IR::Primitive *prim) {
    ...
    if (!count->is<IR::Constant>()) {
        ::P4::error(ErrorType::ERR_UNSUPPORTED, "%1%: Only %2% with a constant value is supported",
                    op1, prim->name);
    ...
    if (number < 0) {
        ::P4::error(ErrorType::ERR_UNSUPPORTED, "%1%: %2% requires a positive amount", op1,
                    prim->name);
        return new IR::Constant(1);
    }
    if (number > 0xFFFF) {
        ::P4::error(ErrorType::ERR_UNSUPPORTED, "%1%: %2% amount is too large", op1, prim->name);
        return new IR::Constant(1);
    }
    return cst;
}
...
CONVERT_PRIMITIVE(count, ) {  // NOLINT(whitespace/parens), remove with C++20 upgrade.
    ...
    if (counter == nullptr) {
        ::P4::error(ErrorType::ERR_EXPECTED, "Expected a counter reference %1%", ref);
        return nullptr;
    }
...
CONVERT_PRIMITIVE(execute_meter, ) {  // NOLINT(whitespace/parens), remove with C++20 upgrade.
    ...
    if (!meter) {
        ::P4::error(ErrorType::ERR_EXPECTED, "Expected a meter reference %1%", ref);
        return nullptr;
    }
    if (!meter->implementation.name.isNullOrEmpty())
        ::P4::warning(ErrorType::WARN_IGNORE_PROPERTY,
                      "Ignoring `implementation' field of meter %1%", meter);
...
CONVERT_PRIMITIVE(modify_field_with_hash_based_offset, ) {  // NOLINT(whitespace/parens), remove
    ...
    if (flc == nullptr) {
        ::P4::error(ErrorType::ERR_EXPECTED, "%1%: Expected a field_list_calculation",
...
CONVERT_PRIMITIVE(register_read, ) {  // NOLINT(whitespace/parens), remove with C++20 upgrade.
    ...
    if (!reg) {
        ::P4::error(ErrorType::ERR_EXPECTED, "Expected a register reference %1%", ref);
        return nullptr;
    }
...
const IR::P4Action *ProgramStructure::convertAction(const IR::ActionFunction *action,
    ...
    for (auto p : action->args) {
        ...
        if (type == IR::Type_Unknown::get()) {
            ::P4::warning(ErrorType::WARN_TYPE_INFERENCE,
...
const IR::Declaration_Instance *ProgramStructure::convert(const IR::Register *reg, cstring newName,
    ...
    if (regElementType) {
        ...
        ::P4::warning(ErrorType::WARN_MISSING, "%1%: Register width unspecified; using %2%", reg,
...
const IR::Declaration_Instance *ProgramStructure::convertDirectMeter(const IR::Meter *m,
    ...
    if (meterOutput == nullptr) {
        ::P4::error(ErrorType::ERR_EXPECTED, "%1%: direct meter with no result", m);
        return nullptr;
    }
...
const IR::P4Control *ProgramStructure::convertControl(const IR::V1Control *control,
    ...
    for (auto c : counters) {
        if (c.first->direct) {
            if (c.first->table.name.isNullOrEmpty()) {
                ::P4::error(ErrorType::ERR_INVALID, "%1%: Direct counter with no table", c.first);
                return nullptr;
            }
            auto tbl = tables.get(c.first->table.name);
            if (tbl == nullptr) {
                ::P4::error(ErrorType::ERR_NOT_FOUND, "Cannot locate table %1%",
                            c.first->table.name);
                return nullptr;
            }
    ...
    for (auto m : meters) {
        if (m.first->direct) {
            if (m.first->table.name.isNullOrEmpty()) {
                ::P4::error(ErrorType::ERR_NOT_FOUND, "%1%: Direct meter with no table", m.first);
                return nullptr;
            }
    ...
    for (auto a : actionsToDo) {
        ...
        if (act == nullptr) {
            ::P4::error(ErrorType::ERR_NOT_FOUND, "Cannot locate action %1%", a);
            return nullptr;
        }
...
void ProgramStructure::createControls() {
    ...
    if (cycles) {
        ...
        ::P4::error(ErrorType::ERR_UNSUPPORTED, "Program contains recursive control blocks");
        return;
    }
...
void ProgramStructure::createMain() {
    ...
    // Add parser_e (empty parser for P4_14 compatibility)
    auto parserEPath = new IR::Path(IR::ID("Parser_E_Impl"));
    auto parserEType = new IR::Type_Name(parserEPath);
    auto parserEConstruct = new IR::ConstructorCallExpression(parserEType, emptyArgs);
    args->push_back(new IR::Argument(parserEConstruct));
...
const IR::FieldList *ProgramStructure::getFieldLists(const IR::FieldListCalculation *flc) {
    ...
    if (flc->input->names.size() == 0) {
        ::P4::error(ErrorType::ERR_UNSUPPORTED, "%1%: field_list_calculation with zero inputs",
                    flc);
        return nullptr;
    }
    if (flc->input->names.size() == 1) {
        ...
        if (result == nullptr)
            ::P4::error(ErrorType::ERR_NOT_FOUND, "Could not find field_list %1%", name);
        return result;
    }
...
const IR::P4Program *ProgramStructure::create(Util::SourceInfo info) {
    ...
    createParser();
    createParserE();
    if (::P4::errorCount()) return nullptr;
    createControls();
    if (::P4::errorCount()) return nullptr;
    createDeparser();
