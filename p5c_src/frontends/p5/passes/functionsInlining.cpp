...
FunctionsInliner::FunctionsInliner(std::vector<cstring> annotations)
    : skipAnnotations(std::move(annotations)) {}
...
bool FunctionsInliner::hasSkipAnnotation(const IR::IAnnotated *node) const {
    if (node == nullptr) return false;
    for (const auto &anno : skipAnnotations) {
...
const IR::Node *FunctionsInliner::preorder(IR::Function *function) {
    if (hasSkipAnnotation(function)) return function;
    if (preCaller()) {
