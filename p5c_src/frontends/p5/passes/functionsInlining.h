...
#ifndef FRONTENDS_P5_FUNCTIONSINLINING_H_
#define FRONTENDS_P5_FUNCTIONSINLINING_H_
...
class FunctionsInliner : public AbstractInliner<FunctionsInlineList, FunctionsInlineWorkList> {
    ...
    std::vector<cstring> skipAnnotations;
    ...
    bool hasSkipAnnotation(const IR::IAnnotated *node) const;

 public:
    explicit FunctionsInliner(std::vector<cstring> annotations = {});
    Visitor::profile_t init_apply(const IR::Node *node) override;
        ...
        passes.push_back(
            new PassRepeated({new TypeChecking(nullptr, typeMap),
                              ...
                              new InlineFunctionsDriver(&functionsToInline,
                                                        new FunctionsInliner(skipAnnotations))
...
#endif /* FRONTENDS_P5_FUNCTIONSINLINING_H_ */
