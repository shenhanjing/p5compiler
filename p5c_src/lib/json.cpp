...
void JsonValue::toC_init(std::ostream &out) const {
    switch (tag) {
        case Kind::String:
            out << "\"" << str << "\"";
            break;
...
void JsonArray::toC_init(std::ostream &out) const {
    out << "[]() -> Json::Value {" << std::endl;
    out << IndentCtl::indent << "Json::Value arr;" << std::endl;
    for (auto v : *this) {
        out << "arr.append(";
        if (v == nullptr)
            out << "Json::nullValue";
...
void JsonObject::toC_init(std::ostream &out) const {
    out << "[]() -> Json::Value {" << IndentCtl::endl;
    out << IndentCtl::indent << "Json::Value obj;" << IndentCtl::endl;
    for (auto &it : *this) {
        out << "obj[\"" << it.first << "\"] = ";
        if (it.second == nullptr)
            out << "Json::nullValue";
        else
            it.second->toC_init(out);
        out << ";" << IndentCtl::endl;
    }
    out << "return obj;" << std::endl;
    out << IndentCtl::unindent << "}()";
}
